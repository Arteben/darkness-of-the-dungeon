...
export class Dude {
  ...
  dudeMoveStateUpdating(newState: DudeStates) {
    ...
    switch (newState) {
      case DudeStates.idle:
        ...
        this.setDydeStaySizes()
        this.calcsForDropAvailable()
        this.sounds.stopLevelSound(SoundLV.dudeMoveSounds)
        break
      case DudeStates.walk:
        this.dudeAnimationKey = {
          key: DudeAnimations.walking, isIgnoreIf: true
        }
        this.sounds.playSingleSoundForLevel(SoundLV.dudeMoveSounds, MoveSounds[MoveSounds.walk])
        break
      case DudeStates.run:
        this.dudeAnimationKey = {
          key: DudeAnimations.run, isIgnoreIf: true
        }
        this.sounds.playSingleSoundForLevel(SoundLV.dudeMoveSounds, MoveSounds[MoveSounds.run])
        break
  ...
  upDownSimpleMoveUpdaiting(
    ...
    const setClimbingSound = () => {
      this.sounds.playSingleSoundForLevel(SoundLV.dudeMoveSounds, MoveSounds[MoveSounds.climbing])
    }
